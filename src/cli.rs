...
impl Session {
    pub fn new(id: &str, sender: mpsc::UnboundedSender<Data>, cli_password: String) -> Self {
        let mut password = "".to_owned();
        if !cli_password.is_empty() {
        ...
        match msgtype {
            ...
            "re-input-password" => {
                ...
                if !self.password.is_empty() {
                    // In headless/automated mode, don't prompt again — password was wrong
                    log::error!("Authentication failed: wrong password provided via --password");
                    self.sender.send(Data::Close).ok();
                } else {
...
pub async fn start_one_port_forward(
    ...
    let handler = Session::new(&id, sender, password);
